...
public class SpannableTextFormat {
    ...
    public static SpanText getColorSpan(SpannableString span, CharSequence text, int color, @NonNull CharSequence... subs) {
        ...
        Texts source = DataCreator.createText(IData.CharSet.English, text);
        for (CharSequence sub : subs) {
            int start, end = 0;
            int count = source.wordCount(sub);
            for (int i = 0; i < count; i++) {
                start = source.indexOf(sub.toString(), end);
                if (start == -1) break;
                /*if (start == 0)*/ colors.add(new TextColor(sub, color, start));
                /*else if (source.getPreviousAlphabetIndex(start) < start - 1)
    ...
    @NonNull
    public static SpanText getLinkSpan(SpannableString span, CharSequence text, StyledView.OnLinkClickListener<?> link, @NonNull CharSequence... subs) {
